<template>
    <div class="bg-white flex flex-col gap-x-3 rounded-md shadow-sm">
        <div class="p-3">
            ...
            <button class="bookmark-btn group" @click="bookmarkItem"
            :class="{
...
export default {
    ...
    methods: {
        ...
        bookmarkItem() {
            let bookmarks = [ ...this._getUserBookmarks ];
    ...
    computed: {
        ...
        alreadyLiked() {
            return this._getUserLikes?.indexOf(this.item.id) > -1
        },
        alreadyBookmarked() {
            return this._getUserBookmarks?.indexOf(this.item.id) > -1
        },
        ...mapGetters(["_getCurrentUser", "_getUserLikes", "_getUserBookmarks"])
    }
}
</script>